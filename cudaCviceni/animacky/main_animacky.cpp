...
int main()
{
    ...
    case 1:
    {
        ...
        cv::Mat CV_Background;
        while (true)
        {
            // vytvoř nový buffer z původního pozadí
            CV_Background = CV_Background_Original.clone();

            // zkopíruj pozadí do CUDA bufferu
            memcpy(cudaBackground.m_p_uchar3, CV_Background.data,
                   SIZEX * SIZEY * sizeof(uchar3));
            ...
            // zobraz obrázek pomocí OpenCV
            cv::imshow("Akvarko", CV_Background);
            if (cv::waitKey(30) >= 0)
                break;
        }
        break;
